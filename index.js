...
Browserify.prototype.require = function (id, opts, isBase) {
    var self = this;
    if (isStream(id)) {
        self.files.push(id);
    ...
    p.on('end', function () {
        process.nextTick(function () {
            ...
            for (var fileName in fileMap) {
              var text = fileMap[fileName].text;
              var path = fileMap[fileName].path;
              fsWrite(path, text);
            };
...
Browserify.prototype._resolve = function (id, parent, cb) {
    ...
    return self._delegateResolve(id, parent, function(err, file, pkg) {
        ...
        if (!file && (self._external[id] || self._external[file])) {
            ...
            // 如果当前项目下找不到对应文件 则去www-server路径下寻找对应位置的模块进行打包
            var _id, _path, dirs, index;

            // 替换路径
            var replaceDirReg = /www-themes+[^]+static+/;

            // 去掉所有 ../ 并且计算出路径层数
            var toRemovePathReg = /[.]{2}\//g;

            // 判断文件后缀
            var fileType = /(\.json|\.html)$/;
            _path = path.dirname(parent.filename);
            _path = _path.replace(replaceDirReg, 'www-server/static');

            // 获得所有父级路径
            dirs = parents(_path);
            index = id.match(toRemovePathReg).length;
            _id = id.replace(toRemovePathReg, '');
            _path = dirs[index];
            file = [_path, _id].join('/');
            ...
            var flag = fsExistsSync(file);

            // 判断www-server下是否存在文件 不存在则报错
            if (!!flag) {
              console.log('www-server下成功找到对应文件', file);
              result(file, pkg);
              return false;
            } else {
              console.log('www-server路径下也找不到对应文件', file);
              return cb(notFound(id, parent.filename));
            };
        } else {

            // 如果当前项目下找不到对应文件 则去www-server路径下寻找对应位置的模块进行打包
            var _id, _path, dirs, index, file2, file3, _basePath;
            ...
            if (id.match(isPathReg)) {
              _path = path.dirname(parent.filename);
              _basePath = path.dirname(parent.filename);
              _path = _path.replace(replaceDirReg, 'www-server/static');

              // 获得所有父级路径
              dirs = parents(_path);

              index = id.match(isPathReg).length - 1;
              _id = id.replace(isPathReg, '');
              _path = dirs[index];
              file2 = [_path, _id].join('/');
              file3 = [_basePath, _id].join('/');
              if (!!!fileType.test(file2)) {
                file2 = file2 + '.js';
                file3 = file3 + '.js';
              };

              var flag = fsExistsSync(file2);

              if (flag) {

                // 获得父类对象 打包时自动继承
                var _superFile = require(file2);
                var _selfFile = require(file);
                var _self;
                _self = new _selfFile();
                _self = _extend(_self, _superFile, file3);
                fsWrite(file3, _self);
              };
            };
        };
    ...
    function findModule (basedir, cb) {}
...
function _extend(self , _super, path) {
  var result;
  var object;
  var _self = self.constructor;
  var _newSuper = new _super;


  var _fn = {};
  var _fnStr;

  for (var key in _newSuper) {
    _fn[key] = _newSuper[key];
  };

  _self.fn = _fn;
  _self.prototype.fn = _fn;

  _fnStr = JSON.stringify(_fn, function(key, val) {
  ...
  var str = ['var app = ' + _self.toString(), 'module.exports = app;'].join('\n');

  // 保存原文件字符串和文件路径 打包完成后还原
  fileMap[path] = {
    text: ['var app = ' + _self.toString(), 'module.exports = app;'].join('\n'),
    path: path
  };

  result = ['var app = ' + _self.toString(), 'app.fn = ' + _fnStr, 'app.prototype.fn = ' + _fnStr, 'module.exports = app;'].join('\n');
  var reg1 = /"function/g;
  var reg2 = /;\\n  }"/g;
  var reg3 = /\\n/g;
  result = result.replace(reg1, 'function').replace(reg2, '}').replace(reg3, '')
  return result;
};
